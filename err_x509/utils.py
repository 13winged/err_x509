...
def create_backup_file(original_path: Path, suffix: str = None) -> Path:
    ...
    backup_path = original_path.with_suffix(
        f".{timestamp}{suffix}{original_path.suffix}"
    )
...
def print_table(headers: List[str], rows: List[List[str]], max_width: int = 80):
    ...
    for i in range(len(headers)):
        max_len = max(
            len(str(headers[i])), max(len(str(row[i])) for row in rows if i < len(row))
        )
        col_widths.append(min(max_len, max_width // len(headers)))

    # Print header
    header_row = " | ".join(str(h).ljust(w)[:w] for h, w in zip(headers, col_widths))
    click.echo(header_row)
    click.echo("-" * len(header_row))

    # Print rows
    for row in rows:
        row_str = " | ".join(str(cell).ljust(w)[:w] for cell, w in zip(row, col_widths))
        click.echo(row_str)
