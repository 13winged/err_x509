...
def fix(input_file: Optional[str], output_file: Optional[str], verbose: bool, 
        backup: bool, force: bool, dry_run: bool):
...
╚═══════════════════════════════════════════════╝
    ...
    if not input_file:
        ...
        for file in default_files:
            if Path(file).exists():
                input_file = file
                click.echo(click.style(f"📁 Using default file: {input_file}", fg='yellow'))
                break
        
        if not input_file:
            click.echo(click.style("❌ No input file specified and no default files found", fg='red'))
            click.echo("\nExamples:")
    ...
    if dry_run:
        ...
        if output_file:
            ...
            click.echo(click.style(f"   Output: {Path(input_file).stem}_fixed.yaml", fg='yellow'))
        sys.exit(0)
    ...
    if success:
        ...
        click.echo(click.style("skip-cert-verify: true disables SSL certificate validation.", fg='white'))
        click.echo(click.style("Use only for testing with trusted servers.", fg='white'))
...
def batch(directory: str, verbose: bool, pattern: str, dry_run: bool):
    ...
    if dry_run:
        for file in files:
            click.echo(click.style(f"   📄 {file.name} → {file.stem}_fixed.yaml", fg='yellow'))
        sys.exit(0)
...
def preview(input_file: str):
    ...
    click.echo(click.style(f"👁️  Preview changes for: {input_file}", fg='cyan', bold=True))
