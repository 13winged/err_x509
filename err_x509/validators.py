...
def validate_file_path(file_path: str) -> Tuple[bool, str]:
    ...
    if path.suffix.lower() not in config.YAML_EXTENSIONS:
        return False, f"File must have YAML extension: {', '.join(config.YAML_EXTENSIONS)}"
    
    # Check file size
    max_size_mb = config.PERFORMANCE_SETTINGS['max_file_size_mb']
    file_size_mb = path.stat().st_size / (1024 * 1024)
    if file_size_mb > max_size_mb:
        return False, config.ERROR_MESSAGES['file_too_large'].format(size=max_size_mb, file=file_path)
...
def validate_proxy_data(proxy: Dict[str, Any]) -> List[str]:
    ...
    if 'type' in proxy and proxy['type'] not in config.CLASH_PROXY_TYPES:
        errors.append(f"Invalid proxy type: {proxy['type']}. Must be one of: {', '.join(config.CLASH_PROXY_TYPES)}")
