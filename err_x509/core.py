...
class X509Fixer:
    ...
    def normalize_config(self, content: str) -> str:
        ...
        patterns = [
            ...
            (
                r"(port:|socks-port:|redir-port:|allow-lan:|mode:|log-level:|external-controller:)",
                r"\n\1",
            ),
        ]
    ...
    def fix_with_yaml(self, content: str) -> Tuple[str, int]:
        ...
        try:
            ...
            if "proxies" in config_data and isinstance(config_data["proxies"], list):
                config_data["proxies"] = [
                    self.process_proxy(p) for p in config_data["proxies"]
                ]
        ...
        if "{" in proxy_text and "}" in proxy_text:
            ...
            if not has_skip_verify:
                ...
                for i in range(len(processed_lines) - 1, -1, -1):
                    if processed_lines[i].strip() and not processed_lines[
                        i
                    ].strip().startswith("#"):
                        # Insert after last property
                        indent = len(processed_lines[i]) - len(
    ...
    def fix_file(
        ...
        try:
            ...
            self.log(
                config.SUCCESS_MESSAGES["proxies_fixed"].format(
                    count=proxy_count, file=output_path.name
                ),
                "success",
                "green",
            )
